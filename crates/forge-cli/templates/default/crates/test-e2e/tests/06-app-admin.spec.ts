...
import { LoginPage, DashboardPage, SelectScopePage, UsersPage, RolesPage, PermissionsPage, AuditLogPage, OrganizationsPage, ProfilePage } from '@/pages'
...
test.describe('App Admin Role', () => {
  ...
  test.describe('6.1 Authentication & Profile Selection', () => {
    test('single profile redirects to dashboard', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        yield* loginPageService.login(SEED_USERS.appAdmin.email, SEED_USERS.appAdmin.password)
  ...
  test.describe('6.10 Generic Entity API (Full CRUD - Global Scope)', () => {
    test('GET /api/entities/organization returns 200 OK (all orgs)', async ({ request }) => {
    ...
    test('GET /api/entities/user returns 200 OK (all users, all orgs)', async ({ request }) => {
  ...
  test.describe('6.11 RPC API (Full CRUD - Global Scope)', () => {
    test('POST /api/rpc entity.list returns 200 OK (all entities, all orgs)', async ({ request }) => {
  ...
  test.describe('6.15 API Token Management', () => {
    ...
    test('use token to GET /api/auth/admin returns 200 OK', async ({ request }) => {
      const tokenResponse = await request.post(`${API_BASE_URL}/api/auth/tokens`)
      const { token } = await tokenResponse.json()
      
      const adminResponse = await request.get(`${API_BASE_URL}/api/auth/admin`, {
