...
import { LoginPage, DashboardPage, SelectScopePage, UsersPage, RolesPage, PermissionsPage, AuditLogPage, ProfilePage } from '@/pages'
...
test.describe('Viewer Role', () => {
  ...
  test.describe('2.1 Authentication & Profile Selection', () => {
    ...
    test('multi-profile redirects to scope selection then dashboard', async ({ page }) => {
      await page.goto('/authentication/login')
  ...
  test.describe('2.7 Organizations Page (Blocked)', () => {
    test('should show 403 when accessing organizations page', async ({ page }) => {
  ...
  test.describe('2.9 API Token Management', () => {
    ...
    test('use token to GET /api/auth/me returns 200 OK', async ({ request }) => {
      // Create token
      const tokenResponse = await request.post(`${API_BASE_URL}/api/auth/tokens`)
  ...
  test.describe('2.10 Generic Entity API (Read-only)', () => {
    test('GET /api/entities/user returns 200 OK', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/user`)
      expect(response.status()).toBe(200)
    })

    test('GET /api/entities/role returns 200 OK', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/role`)
      expect(response.status()).toBe(200)
    })

    test('GET /api/entities/permission returns 200 OK', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/permission`)
      expect(response.status()).toBe(200)
    })

    test('GET /api/entities/audit returns 200 OK', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/audit`)
      expect(response.status()).toBe(200)
    })

    test('GET /api/entities/organization returns 403 Forbidden', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/organization`)
      expect(response.status()).toBe(403)
    })

    test('POST /api/entities/organization returns 403 Forbidden', async ({ request }) => {
  ...
  test.describe('2.11 RPC API (Read-only)', () => {
    test('POST /api/rpc entity.list returns 200 OK', async ({ request }) => {
    ...
    test('POST /api/rpc entity.get returns 200 OK', async ({ request }) => {
      // First get a user ID
      const listResponse = await request.post(`${API_BASE_URL}/api/rpc`, {
        data: { method: 'entity.list', params: { entity: 'user' } },
      })
      ...
      if (listData.result && listData.result.length > 0) {
        ...
        const response = await request.post(`${API_BASE_URL}/api/rpc`, {
          data: { method: 'entity.get', params: { entity: 'user', id: userId } },
        })
        expect(response.status()).toBe(200)
      }
    })

    test('POST /api/rpc entity.create returns 403 Forbidden', async ({ request }) => {
      const response = await request.post(`${API_BASE_URL}/api/rpc`, {
