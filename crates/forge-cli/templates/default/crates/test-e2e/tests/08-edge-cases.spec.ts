...
import { DashboardPage, UsersPage, RolesPage, OrganizationsPage, AuditLogPage } from '@/pages'
...
test.describe('Edge Cases & Boundary Conditions', () => {
  test.describe('8.1 Empty States', () => {
    ...
    test('organizations page shows empty state', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        const emptyState = page.locator('[data-testid="organizations-empty-state"]')
  ...
  test.describe('8.2 Large Datasets', () => {
    ...
    test('large user list pagination works correctly', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        if (paginationCount > 0) {
          ...
          const nextButton = page.locator('[data-testid="users-pagination-next"]')
    ...
    test('large organization list pagination works correctly', async ({ page }) => {
      await page.goto('/dashboard/organizations')
      ...
      const program = Effect.gen(function* () {
        ...
        // Check if pagination exists
        const pagination = page.locator('[data-testid="organizations-pagination"]')
        const paginationCount = yield* LocatorHelpers.count(pagination)
        if (paginationCount > 0) {
          yield* ExpectHelpers.toBeVisible(pagination)
          
          const nextButton = page.locator('[data-testid="organizations-pagination-next"]')
          const nextButtonCount = yield* LocatorHelpers.count(nextButton)
          const nextButtonEnabled = yield* LocatorHelpers.isEnabled(nextButton)
          if (nextButtonCount > 0 && nextButtonEnabled) {
            yield* LocatorHelpers.click(nextButton)
    ...
    test('large audit log pagination works correctly', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        if (paginationCount > 0) {
          ...
          const nextButton = page.locator('[data-testid="audit-log-pagination-next"]')
          const nextButtonCount = yield* LocatorHelpers.count(nextButton)
          const nextButtonEnabled = yield* LocatorHelpers.isEnabled(nextButton)
          if (nextButtonCount > 0 && nextButtonEnabled) {
            yield* LocatorHelpers.click(nextButton)
    ...
    test('cursor pagination works correctly', async ({ request }) => {
      ...
      const response = await request.get(`${API_BASE_URL}/api/entities/user?limit=10`)
      ...
      if (data.cursor) {
        const nextResponse = await request.get(`${API_BASE_URL}/api/entities/user?limit=10&cursor=${data.cursor}`)
  ...
  test.describe('8.3 Invalid Inputs', () => {
    ...
    test('invalid email format shows validation error', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        // Try invalid email
        const emailInput = page.locator('[data-testid="user-create-email-input"]')
        yield* LocatorHelpers.fill(emailInput, 'invalid-email')
        
        const submitButton = page.locator('[data-testid="user-create-submit-button"]')
        yield* LocatorHelpers.click(submitButton)
        
        // Check for validation error
        const errorMessage = page.locator('[data-testid="user-create-email-error"]')
    ...
    test('short password shows validation error', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        // Try short password
        const passwordInput = page.locator('[data-testid="user-create-password-input"]')
        ...
        // Check for validation error
        const errorMessage = page.locator('[data-testid="user-create-password-error"]')
        const errorCount = yield* LocatorHelpers.count(errorMessage)
        if (errorCount > 0) {
          yield* ExpectHelpers.toBeVisible(errorMessage)
        }
    ...
    test('invalid UUID returns 400 Bad Request', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/user/invalid-uuid`)
      expect(response.status()).toBe(400)
    })

    test('invalid filter JSON returns 400 Bad Request', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/user?filter=invalid-json`)
      expect(response.status()).toBe(400)
    })

    test('invalid sort field returns 400 Bad Request', async ({ request }) => {
      const response = await request.get(`${API_BASE_URL}/api/entities/user?sort=invalid-field`)
      expect(response.status()).toBe(400)
    })
  })
  ...
  test.describe('8.4 Network Conditions', () => {
    ...
    test('slow network shows loading states', async ({ page, context }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        const loadingSpinner = page.locator('[data-testid="users-loading-spinner"]')
    ...
    test('network error shows error message', async ({ page, context }) => {
      ...
      const errorMessage = page.locator('[data-testid="error-message"]').or(page.locator('text=/error/i'))
    ...
    test('timeout shows error message', async ({ page, context }) => {
      ...
      const errorMessage = page.locator('[data-testid="error-message"]').or(page.locator('text=/timeout/i'))
  ...
  test.describe('8.5 Browser Compatibility', () => {
    ...
    test('mobile viewport responsive layout works', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        const hamburgerMenu = page.locator('[data-testid="sidebar-hamburger-menu"]')
    ...
    test('dark/light theme toggle works', async ({ page }) => {
      ...
      const program = Effect.gen(function* () {
        ...
        const hasLightClass = yield* Effect.promise<boolean>(() => 
          body.evaluate((el) => 
            !el.classList.contains('dark') || el.getAttribute('data-theme') === 'light'
          )
        )
