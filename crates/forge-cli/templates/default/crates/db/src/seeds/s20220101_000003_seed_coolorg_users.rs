...
async fn create_organization(_db: &DbConnection, name: &str, slug: &str) -> Result<Uuid, Box<dyn std::error::Error>> {
  let base = base_url();
...
async fn create_role(organization_id: Uuid, name: &str) -> Result<(), Box<dyn std::error::Error>> {
  let base = base_url();
  let url = format!(
...
pub async fn seed(_db: &DbConnection) -> Result<(), Box<dyn std::error::Error>> {
  let org_id = create_organization(_db, "CoolOrg", "coolorg").await?;
