...
export interface LiveWsConnection {
	...
	readonly subscribe: (key: LiveUpdateKey, listener: (data: unknown) => void) => () => void;
 ...
 */
export const getConnection = Effect.flatMap(LiveWs, (liveWs) => Effect.succeed(liveWs));
 ...
 */
export const subscribe = (key: LiveUpdateKey, listener: (data: unknown) => void) =>
	Effect.flatMap(LiveWs, (liveWs) => Effect.succeed(liveWs.subscribe(key, listener)));
 ...
 */
export const isConnected = Effect.flatMap(LiveWs, (liveWs) => Effect.succeed(liveWs.connected));
...
export const makeLiveWs = (connection: {
	connected: boolean;
	subscribe: (key: LiveUpdateKey, listener: (data: unknown) => void) => () => void;
 ...
 */
export const LiveWsTest = Layer.succeed(LiveWs, makeLiveWs({
	connected: false,
	subscribe: () => () => {},
	send: () => Promise.reject(new Error("WebSocket not available in test context"))
}));