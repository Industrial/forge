...
function toSnapshot(s: MutableSnapshot): AuthStateSnapshot {
	return new AuthStateSnapshot({
		...s,
		user: s.user,
		profiles: s.profiles.map((p) => (p instanceof Profile ? p : new Profile(p))),
		flash: s.flash != null && !(s.flash instanceof Flash) ? new Flash(s.flash) : s.flash,
	});
}
...
function makeAuthenticationStoreMock(
	...
	if (options.initialState) {
		...
		if (i.user !== undefined) state.user = i.user === null ? null : i.user instanceof AuthUser ? i.user : new AuthUser(i.user);
		if (i.profiles !== undefined) state.profiles = [...i.profiles];
		if (i.permissions !== undefined) state.permissions = [...i.permissions];
		if (i.flash !== undefined) state.flash = i.flash === null ? null : i.flash instanceof Flash ? i.flash : new Flash(i.flash);
		...
		if (i.currentRoleName !== undefined) state.currentRoleName = i.currentRoleName;
		if (i.needs_profile_select !== undefined) state.needs_profile_select = i.needs_profile_select;
	}
 ...
 */
export const AuthenticationStoreMock = (options?: AuthenticationStoreMockOptions) =>
