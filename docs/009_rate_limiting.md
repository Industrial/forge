...
**Conditional throttling from config (generated by `forge new`):**

`forge new` generates an app that checks `config.app.environment` and only then applies the rate limiters:
...
When `config/app.toml` has `[app] environment = "development"` or `"test"`, no rate limiting is applied. When `environment = "production"`, both per-IP and per-user limits are active.
