...
export function assertProjectLayout(): void {
  ...
  const mainRs = fs.readFileSync(path.join(root, 'crates/app/src/main.rs'), 'utf-8');
  if (!mainRs.includes('App::new()')) {
    throw new Error('main.rs should use App::new()');
  }
  if (!mainRs.includes('.serve()') && !mainRs.includes('into_router_before_state')) {
...
export function assertAuthLayout(): void {
  ...
  if (!userModel.includes('current_org_id') || !userModel.includes('current_role')) {
  ...
  if (!hasAdmin) {
    throw new Error('auth handlers should gate admin on is_admin or record_authz_denied');
  }
